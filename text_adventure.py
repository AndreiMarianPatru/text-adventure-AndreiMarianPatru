...
for i in range(10):
    if i==0:
        a={'apple':0,'box':0,'bullet':0,'map':0,'knife':0,'THE GOLDEN DONUTS':0}
    else:
...
def lookroom():
    ...
    if i==0:
        time.sleep(1)
        print("There is nothing here, go and explore something")
    else:
...
def look_item(key):
    ...
    if key not in k :
        ...
        if key=='apple':
            ...
            time.sleep(1)
            print('You are looking at a gun')
...
while True:
    ...
    if command=="w":
        ...
        time.sleep(1)
        if split[1] in playerInv or room[currentRoom]:
            if playerInv.get(split[1])>0 or room[currentRoom].get(split[1])>0:
                look_item(split[1])
            else:
                time.sleep(1)
                print("There is no such item in your inventory or in the current room")
        else:
            print("There is no such thing in your room or your inventory")
    elif split[0]=='drop':
        time.sleep(1)
        drop_item(split[1])
        print('You dropped a/an ',split[1])
    elif split[0]=='use':
        time.sleep(1)
        item=split[1]
        j=list(playerInv.keys())
